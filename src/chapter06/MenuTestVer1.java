 ...
 *  - 메뉴는 피자, 햄버거, 라멘, 샐러드
 *  - 메뉴명, 이모지, 가격을 저장
 *  - 메뉴명, 이모지, 가격은 각각 1차원의 배열로 생성하여 관리
 *  - 메뉴 관리를 위해 각 배열의 주소를 통일시킨다.
 */
public class MenuTestVer1 {
	...
	public static void main(String[] args) {
		...
		while(menuFlag) {
			System.out.println("---------------------------------------------");
			System.out.println("\t더 조은 레스트랑 메뉴 관리 프로그램");
			System.out.println("---------------------------------------------");
			...
			//메뉴 체크
				if(menu ==1) {
					// 저장공간의 크기 만큼 반복수행, 메뉴 하나 등록 완료시 카운트 변수 1 증가
			    	...
			    	if(menuCount != 0) {
				    	...
				    	while(searchFlag) {
				    		...
				    		System.out.println("계속 검색?(계속:아무키나 누르세요, n:종료) >");
				    		if(scan.next().equals("n")) {
				    			searchFlag = false;
				    		} else {
